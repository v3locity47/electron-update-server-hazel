...
module.exports = ({ cache, config }) => {
  ...
  exports.releases = async (req, res) => {
    ...
    if (!latest.files || !latest.files['releases.win.json']) {
      res.statusCode = 204
      res.end()
    ...
    const content = latest.files['releases.win.json']
  ...
  exports.getWinFiles = async (req, res) => {
    const { file: fileName } = req.params

    // Get the latest version from the cache
    const { assets, platforms } = await loadCache()
    const requestedAsset = assets.filter(asset => asset.name === fileName)

    if (!requestedAsset || requestedAsset.length === 0) {
      send(res, 404, 'Requested Asset not Found!')
      return
    }

    const asset = requestedAsset[0]
    if (shouldProxyPrivateDownload) {
